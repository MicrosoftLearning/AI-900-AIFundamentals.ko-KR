...
*개체 감지*는 이미지에 있는 개체의 개별 인스턴스를 분류하고 위치를 표시하는 경계 상자를* 나타내*도록 기계 학습 모델을 학습하는 컴퓨터 비전의 한 형태입니다. 이를 이미지 분류 *(모델이 "이 이미지란?"라는 질문에 대답하는 경우)에서 *모델에게 "이 이미지에 있는 개체는 무엇이며 어디에 있습니까?"라고 물어볼 수 있는 솔루션 빌드에 대한 진행으로 생각할 수 있습니다.
...
Microsoft Azure의 **Custom Vision** 서비스는 사용자 지정 개체 검색 모델을 만들고 게시하기 위한 클라우드 기반 솔루션을 제공합니다. Azure에서 Custom Vision 서비스를 사용하여 기존 이미지를 기반으로 개체 검색 모델을 학습할 수 있습니다. 개체 검색 솔루션을 만드는 데는 두 가지 요소가 있습니다. 먼저 레이블이 지정된 이미지를 사용하여 개체의 위치 및 클래스를 검색하도록 모델을 학습시켜야 합니다. 그런 다음, 모델이 학습되면 애플리케이션에서 사용할 수 있는 서비스로 게시해야 합니다.
...
1. [https://aka.ms/traffic-images](https://aka.ms/traffic-images)에서 학습 이미지를 다운로드하고 압축을 풉니다. 추출된 폴더에는 자전거 운전자와 보행자의 이미지 컬렉션이 포함되어 있습니다.
...
1. 오른쪽의 **다음** (**>)** 링크를 사용하여 다음 이미지로 이동하고 해당 개체에 태그를 지정합니다. 그런 다음 전체 이미지 컬렉션을 계속 작업하고 각 자전거와 보행자에 태그를 지정합니다.
...
1. Custom Vision 프로젝트에서 **학습**을 클릭하여 태그가 지정된 이미지로 개체 감지 모델을 학습시킵니다. **빠른 학습** 옵션을 선택합니다.
    ...
    > **팁**: 학습에 몇 분 정도 걸릴 수 있습니다. 기다리는 동안 도로 안전 개선 이니셔티브에서 [컴퓨터 비전을 사용하는 실제 프로젝트를 설명하는 스마트 도시를](https://www.microsoft.com/research/video/video-analytics-for-smart-cities/) 위한 비디오 분석을 검사.

2. 학습이 완료되면 정밀도, 재현율* 및 *mAP* 성능 메트릭을 검토*합니다. 이러한 메트릭은 개체 검색 모델의 예측 성능을 측정하며 모두 합리적으로* 높아야 합니다. * 
...
4. 페이지 오른쪽 위에서 빠른 테스트를 클릭한 **다음 **이미지 URL** 상자에 결과를 입력`https://aka.ms/pedestrian-cyclist`** 하고 봅니다.
...
1. Azure Portal이 포함된 브라우저 탭으로 다시 전환하고 검색 상자 오른쪽에 있는 페이지 맨 위에 있는 Cloud Shell(**[>_]**) 단추를 선택합니다 **.** 그러면 포털 아래쪽에 클라우드 셸 창이 열립니다.
    ...
    ![Azure Portal의 클라우드 셸 스크린샷](media/create-object-detection-solution/cloud-shell.png)
