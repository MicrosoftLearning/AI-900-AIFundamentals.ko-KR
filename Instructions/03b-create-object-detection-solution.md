...
Microsoft Azure의 **Custom Vision** 서비스는 사용자 지정 개체 검색 모델을 만들고 게시하기 위한 클라우드 기반 솔루션을 제공합니다. Azure에서는 Custom Vision 서비스를 사용하여 기존 이미지를 기반으로 개체 검색 모델을 학습시킬 수 있습니다. 개체 검색 솔루션을 만드는 데는 두 가지 요소가 있습니다. 먼저 레이블이 지정된 이미지를 사용하여 개체의 위치 및 클래스를 검색하도록 모델을 학습시켜야 합니다. 그런 다음, 모델이 학습되면 애플리케이션에서 사용할 수 있는 서비스로 게시해야 합니다.
...
1. Custom Vision 포털의 **Traffic Safety** 개체 검색 프로젝트에서 **이미지 추가**를 선택하고 추출된 폴더에 있는 모든 이미지를 업로드합니다.
...
1. 자동으로 감지된 영역이 표시될 때까지 이미지의 모든 개체(자전거 또는 보행자)에 마우스를 놓습니다. 그런 다음, 개체를 선택하고 필요한 경우 영역의 크기를 조정하여 개체를 둘러싸도록 합니다. 또는 단순히 개체를 주변을 끌어 영역을 만들 수 있습니다.
...
1. 오른쪽의 **다음****(>)** 링크를 사용하여 다음 이미지로 이동하고 해당 개체에 태그를 지정합니다. 그런 다음 전체 이미지 컬렉션을 계속 작업하고 각 자전거 및 보행자에 태그를 지정합니다.

    이미지에 태그를 지정할 때 다음 사항에 유의하세요.

    - 일부 이미지에는 여러 개체가 포함되어 있으며 잠재적으로 형식이 다를 수 있습니다. 겹치는 경우에도 각각에 태그를 지정합니다.
...
2. 학습이 완료되면 *정밀도*, *재현*율 및 *mAP* 성능 메트릭을 검토합니다. 이러한 메트릭은 개체 감지 모델의 예측 성능을 측정하며 모두 합리적으로 높아야 합니다.
...
4. 페이지 오른쪽 위에서 **빠른 테스트를** 클릭한 다음 **이미지 URL** 상자에 결과를 입력 `https://aka.ms/pedestrian-cyclist` 하고 확인합니다.

    오른쪽 창의 **예측** 아래에 검색된 각 개체가 태그와 확률로 나열됩니다. 각 개체를 선택하여 이미지에서 강조 표시된 개체를 확인합니다.

    예측된 개체가 모두 올바르지 않을 수 있습니다. 결국 자전거 운전자와 보행자는 많은 공통 기능을 공유합니다. 모델이 가장 확신하는 예측은 확률 값이 가장 높습니다. **임계값** 슬라이더를 사용하여 확률이 낮은 개체를 제거합니다. 올바른 예측만 포함되는 지점을 찾을 수 있어야 합니다(약 85~90%).
...
이제 샘플 클라이언트 애플리케이션을 사용하여 이미지에서 자전거 및 보행자를 감지할 수 있습니다.
...
1. PowerShell 창에서 다음 명령을 입력하여 코드를 실행합니다.
    ...
    이 코드는 모델을 사용하여 다음 이미지에서 개체를 검색합니다.
...
바라건대, 당신의 개체 감지 모델은 테스트 이미지에서 보행자와 자전거 를 감지하는 좋은 일을했다.
