...
1. [Azure Machine Learning 스튜디오](https://ml.azure.com?azure-portal=true) 화면 왼쪽 위에 있는 메뉴 아이콘을 선택하여 왼쪽 창을 확장합니다. **데이터** 페이지(자산** 아래**)를 선택합니다. 데이터 페이지에는 Azure ML에서 사용할 특정 데이터 파일 또는 테이블이 있습니다. 이 페이지에서도 데이터 세트를 만들 수 있습니다.
...
## 디자이너에서 파이프라인 만들기 및 캔버스에 데이터 로드

Azure Machine Learning 디자이너는 시작하려면 먼저 파이프라인을 만들고 작업하려는 데이터 세트를 추가해야 합니다.

1. Azure Machine Learning 스튜디오 왼쪽 창에서 [디자이너** 항목(작성 중 **)을** 선택한 **다음 새 파이프라인을 만들도록 선택합니다**+**.](https://ml.azure.com?azure-portal=true)

1. 초안 이름을 Pipeline-Created-on-*date ***에서 **Diabetes Training**으로 **변경합니다.
...
1. **데이터**를 선택합니다. **diabetes-data** 데이터 세트를 찾아 캔버스에 배치합니다.

1. 캔버스에서 당뇨병 데이터 데이터** 세트를 마우스 오른쪽 단추로 클릭하고(Mac에서 Ctrl+클릭) **데이터** 미리 보기를 선택합니다**.
...
1. 다음과 같이 캔버스에서 **데이터 세트를 볼 수 있도록 DataOutput** 탭을 닫습니다.
...
1. 왼쪽의 **자산 라이브러리** 창에서 데이터 변환 및 모델 학습에 사용할 수 있는 다양한 모듈이 포함된 구성 요소를** 선택합니다**. 검색 창을 사용해 모듈을 빠르게 찾을 수도 있습니다.

    ![디자이너 자산 라이브러리, 검색 창, 구성 요소 아이콘 위치의 스크린샷.](media/create-classification-model/designer-asset-library-components.png)

1. **데이터 세트** 모듈에서 열 선택 모듈을 찾아 당뇨병 데이터** 세트 아래**의 캔버스에 배치합니다. 그런 다음 **diabetes-data** 데이터 세트 하단의 출력을 **데이터 세트에서 열 선택** 모듈 상단의 입력에 연결합니다.

1. **데이터 세트에서 열 선택** 모듈을 두 번 클릭하여 오른쪽에 있는 설정 창에 액세스합니다. **열 편집**을 선택합니다. 그런 다음 열** 선택 창에서 **이름으로** **선택하고 **모든** 열을 추가합니다. 그런 다음 PatientID**를 제거하고 **저장**을 클릭합니다**.

1. 데이터 정규화 모듈을 **찾아서 데이터** 세트** 모듈의 **열 선택 아래 캔버스에 배치합니다. 그런 다음 **데이터 세트에서 열 선택** 모듈 하단의 출력을 다음과 같이 **데이터 정규화** 모듈 상단의 입력에 연결합니다.
...
1. True로 *검사* 경우 변환 메서드*를 **MinMax**로 *설정하고 상수 열에 대해 0을 **사용합니다.** 열 편집을 사용하여 변환 **할 열을 편집합니다**. 규칙이** 있는 **열을 선택하고 다음 목록을 복사하여 포함 열 이름 아래에 붙여넣습니다.  
...
1. 기본 페이지에서 새로** 만들기를 선택하고 **실험의 이름을 mslearn-diabetes-training**으로 **설정한 다음, 다음**을 선택합니다**.** ** 
...
1. 실행이 완료될 때까지 몇 분 정도 기다립니다. 자산에서 작업을 선택하여 **** 작업의 상태 검사 수 있습니다 **.** 거기에서 mslearn-diabetes-training** 실험을 선택한 다음 **당뇨병 훈련** 작업을 선택합니다**.

## 변환된 데이터 보기

실행이 완료되면 이제 모델 학습을 위해 데이터 세트가 준비됩니다.

1. 캔버스에서 데이터** 정규화 모듈을 마우스 오른쪽 단추로 클릭하고(Mac에서 Ctrl+클릭) **데이터** 미리 보기를 선택합니다**. **변환된 데이터 집합**을 선택합니다.
...
1. 디자이너** 페이지로 돌아가**서 Diabetes Training** 파이프라인을 **선택합니다.
...
1. **자산 라이브러리**에서 **모델 학습** 모듈을 찾아 **데이터 분할** 모듈 아래의 캔버스에 배치합니다. 그런 다음 분할** 데이터 모듈의 **결과 데이터 세트1 *(왼쪽) 출력을 *모델** 학습 모듈의 데이터 세트*(오른쪽) 입력에 **연결*합니다.
...
1. 구성 및 제출**을 선택하고 **mslearn-diabetes-training**이라는 **기존 실험을 사용하여 파이프라인을 실행합니다.

1. 실험이 완료될 때까지 기다립니다. 5분 이상 걸릴 수 있습니다.

1. 자산에서 작업을 선택하여 **** 작업의 상태 확인합니다 **.** 거기에서 mslearn-diabetes-training **** 실험을 선택한 다음 최신 **당뇨병 훈련** 작업을 선택합니다.

1. 새 탭에서 캔버스에서 모델** 점수 매기기 모듈을 마우스 오른쪽 단추로 클릭하고(Mac에서 Ctrl+클릭) **데이터** 미리 보기를 선택한 **다음 점수 매기기 데이터 세트를** 선택하여 **결과를 봅니다.
...
1. 디자이너로 돌아가**서 만든 Diabetes Training** 파이프라인을 엽니다**.**
...
1. 새 탭에서 캔버스에서 모델** 평가 모듈을 마우스 오른쪽 단추로 클릭하고(Mac에서 Ctrl+클릭) **데이터 미리 보기를** 선택한 **다음, 평가 결과를** 선택하여 **성능 메트릭을 봅니다. 이러한 메트릭은 데이터 과학자가 유효성 검사 데이터를 기준으로 모델이 얼마나 잘 예측하는지를 평가하는 데 도움이 될 수 있습니다.

1. 아래로 스크롤하여 모델의 혼동 행렬*을 봅니*다. 가능한 각 클래스에 대한 예측 및 실제 값 수를 관찰합니다. 

1. 다음이 포함된 혼동 행렬의 왼쪽에서 메트릭을 검토합니다.
    - **정확도**: 즉, 모델이 당뇨병을 올바르게 예측하는 비율은 어느 정도입니까?
    - **정밀도**: 즉, 모델이 당뇨병을 앓고 있다고 *예측한* 모든 환자 중에서 모델이 올바른 시간의 비율입니다. 
    - **기억하십시오**: 다른 말로, 실제로* 당뇨병이 있는 모든 환자*에서, 얼마나 많은 당뇨병 케이스는 모형이 정확하게 확인했습니까?
    - **F1 점수**
...
1. 캔버스 위의 메뉴를 찾아 유추 파이프라인** 만들기를 선택합니다**. 메뉴에서 **유추 파이프라인 만들기**를 찾으려면 화면을 전체 화면으로 확장하고 화면의 오른쪽 위 모서리에 있는 세 개의 점 아이콘 **...** 을 클릭해야 할 수 있습니다.  

    ![유추 파이프라인 만들기의 위치 스크린샷](media/create-classification-model/create-inference-pipeline.png)

1. **유추 파이프라인** 만들기 드롭다운 목록에서 실시간 유추 파이프라인**을 선택합니다**. 몇 초 후에 **당뇨병 학습-실시간 유추**라는 새 버전의 파이프라인이 열립니다.

1. 새 파이프라인의 이름을 **당뇨병 예측**으로 바꾸고 새 파이프라인을 검토합니다. 일부 변환 및 학습 단계가 이 파이프라인의 일부입니다. 학습된 모델은 새 데이터의 점수를 매기는 데 사용됩니다. 파이프라인에는 결과를 반환하는 웹 서비스 출력도 포함되어 있습니다.
...
1. 파이프라인에는 사용자 지정 데이터 세트에서 만든 모델에 대한 **웹 서비스 입력** 구성 요소가 자동으로 포함되지 않습니다. 자산 라이브러리에서 **웹 서비스 입력** 구성 요소를 검색하여 파이프라인의 맨 위에 배치합니다. 의 출력 ** 을 커넥트캔버스에 **이미 있는 데이터 세트** 구성 요소의 열 선택 구성 요소에 대한 웹 서비스 입력** 구성 요소입니다.
...
1. 새 ** 커넥트데이터** 세트의 열 선택 모듈과 웹 서비스** 입력과 **동일한 **데이터 세트**** 입력에 수동으로 데이터 모듈을 **입력합니다.

1. **데이터 세트에서 열 선택** 모듈을 편집합니다. 선택한 열에서 *당뇨병** 환자를 제거**합니다*.
...
1. 에서 출력 ** 을 커넥트모델** 모듈을 *Python 스크립트 실행의 **Dataset1*(가장 왼쪽) 입력에 채점하고 Python 스크립트**** 실행 모듈의 **결과 데이터 세트 *(왼쪽) 출력을 웹 서비스 출력**에 **연결*합니다.
...
1. 작업** 탭으로 **돌아갑니다. 여기에서 **mslearn-diabetes-inference** 실험을 선택한 다음, 당뇨병** 예측 작업을 선택합니다**.

1. 파이프라인이 완료되면 Python 스크립트** 실행 모듈을 **선택합니다. **데이터 미리 보기**를 선택하고 **결과 데이터 세트**를 선택하여 입력 데이터에서 세 환자 관측에 대해 예측한 레이블 및 확률을 표시합니다.
...
1. 당뇨병** 예측 작업 창의 **맨 위에서 배포**를 선택합니다**.

    ![자동 가격 예측 유추 파이프라인에 대한 배포 단추의 스크린샷](media/create-classification-model/deploy-screenshot.png)

1. **실시간 엔드포인트 설정에서 새 실시간 엔드포인트**** 배포를 선택하고 **다음 설정을 사용합니다.
    - **이름**: predict-diabetes
    - **설명**: 당뇨병을 분류합니다.
    - **컴퓨팅 형식**: Azure Container Instances

1. 배포**를 선택하고 **웹 서비스가 배포될 때까지 기다립니다. 몇 분 정도 걸릴 수 있습니다.
