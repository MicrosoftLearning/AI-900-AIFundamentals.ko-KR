...
## Designer 파이프라인 만들기 및 캔버스에 데이터 로드

Azure Machine Learning 디자이너는 시작하려면 먼저 파이프라인을 만들고 작업하려는 데이터 세트를 추가해야 합니다.

1. [Azure Machine Learning 스튜디오](https://ml.azure.com?azure-portal=true) 왼쪽 창에서 **Designer** 항목(작성 아래)**을** 선택한 다음 를 선택하여 **+** 새 파이프라인을 만듭니다.
...
1. **데이터** 선택 **diabetes-data** 데이터 세트를 찾아 캔버스에 배치합니다.

1. 캔버스에서 **diabetes-data 데이터** 세트를 마우스 오른쪽 단추로 클릭하고(Mac에서 Ctrl+클릭) **데이터 미리 보기를** 선택합니다.
...
1. **데이터 세트에서 열 선택 모듈을 찾아****서 diabetes-data 데이터** 세트 아래의 캔버스에 배치합니다. 그런 다음 **diabetes-data** 데이터 세트 하단의 출력을 **데이터 세트에서 열 선택** 모듈 상단의 입력에 연결합니다.
...
1. **데이터 정규화** 모듈을 찾아 데이터 **세트에서 열 선택** 모듈 아래의 캔버스에 배치합니다. 그런 다음 **데이터 세트에서 열 선택** 모듈 하단의 출력을 다음과 같이 **데이터 정규화** 모듈 상단의 입력에 연결합니다.
...
1. **기본 페이지에서** **새로 만들기**를 선택하고 실험 이름을 **mslearn-diabetes-training으로** 설정한 다음 **, 다음**을 선택합니다.
...
1. 실행이 완료될 때까지 몇 분 정도 기다립니다. 자산**에서 작업을** 선택하여 작업의 상태 검사 수 **있습니다**. 여기에서 **mslearn-diabetes-training** 실험을 선택한 다음 **, 당뇨병 훈련** 작업을 선택합니다.
...
1. **Designer** 페이지로 돌아가**서 Diabetes Training 파이프라인을** 선택합니다.
...
1. **& 제출 구성**을 선택하고 **mslearn-diabetes-training**이라는 기존 실험을 사용하여 파이프라인을 실행합니다.

1. 실험이 완료될 때까지 기다립니다. 5분 이상 걸릴 수 있습니다.

1. 자산 **아래에서 작업을** 선택하여 작업의 상태 확인**합니다**. 여기에서 **mslearn-diabetes-training** 실험을 선택한 다음, 최신 **당뇨병 교육** 작업을 선택합니다.

1. 새 탭에서 캔버스에서 **모델 점수 매기** 기 모듈을 마우스 오른쪽 단추로 클릭하고(Mac에서 Ctrl+클릭) **데이터 미리 보기를** 선택한 다음 **점수가 매표된 데이터 세트를** 선택하여 결과를 봅니다.
...
1. **Designer** 돌아가서 만든 **Diabetes Training 파이프라인을** 엽니다.
...
1. **유추 파이프라인 만들기** 드롭다운 목록에서 **실시간 유추 파이프라인을** 선택합니다. 몇 초 후에 **당뇨병 학습-실시간 유추**라는 새 버전의 파이프라인이 열립니다.
...
1. **모델 점수 매기**기 모듈의 출력을 **Python 스크립트 실행**의 *Dataset1*(왼쪽) 입력에 연결하고 **Python 스크립트 실행** 모듈의 *결과 데이터 세트*(왼쪽) 출력을 **웹 서비스 출력**에 연결합니다.
...
1. **실시간 엔드포인트 설정**에서 **새 실시간 엔드포인트 배포를** 선택하고 다음 설정을 사용합니다.
    - **이름**: predict-diabetes
    - **설명**: 당뇨병을 분류합니다.
