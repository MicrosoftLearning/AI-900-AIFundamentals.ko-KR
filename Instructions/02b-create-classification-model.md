...
1. Azure Machine Learning 스튜디오에서 새로 만든 작업 영역이 표시됩니다. 그렇지 않은 경우 왼쪽 메뉴에서 Azure 디렉터리를 선택합니다. 그런 다음 새 왼쪽 메뉴에서 디렉터리에 연결된 모든 작업 영역이 나열되는 **작업 영역**을 선택하고, 이 연습을 위해 만든 작업 영역 하나를 선택합니다.
...
1. [Azure Machine Learning 스튜디오](https://ml.azure.com?azure-portal=true)에서 왼쪽 위에 있는 **≡**(세 줄) 아이콘을 선택하여 인터페이스의 다양한 페이지를 봅니다(화면 크기를 최대화해야 할 수 있음). 왼쪽 창에서 해당 페이지를 사용하여 작업 영역에서 리소스를 관리할 수 있습니다. **컴퓨팅** 페이지(**관리** 아래)를 선택합니다.
...
1. [Azure Machine Learning 스튜디오](https://ml.azure.com?azure-portal=true) 화면에서 왼쪽 위에 있는 메뉴 아이콘을 선택하여 왼쪽 창을 확장합니다. **데이터** 페이지(**자산** 아래)를 선택합니다. 데이터 페이지에는 Azure ML에서 사용할 특정 데이터 파일 또는 테이블이 있습니다. 이 페이지에서도 데이터 세트를 만들 수 있습니다.
...
1. [Azure Machine Learning Studio](https://ml.azure.com?azure-portal=true)에서 왼쪽 창의 **작성** 아래에 있는 **디자이너** 아이템을 확인하고 **+** 를 선택하여 새 파이프라인을 만듭니다.
...
1. 캔버스의 **diabetes-data** 데이터 세트를 마우스 오른쪽 버튼으로 클릭(Mac에서는 Ctrl+클릭)하고 **데이터 미리 보기**를 선택합니다.
...
1. **데이터 세트에서 열 선택** 모듈을 두 번 클릭하여 오른쪽에 있는 설정 창에 액세스합니다. **열 편집**을 선택합니다. 그런 다음 **열 선택** 창에서 **이름별**을 선택하고 **모두 추가**를 선택합니다. 그런 다음 **PatientID**를 제거하고 **저장**을 클릭합니다.
...
1. *변환 메서드*를 **MinMax**로 설정하고 *선택한 경우 상수 열에 0 사용*을 **True**로 설정합니다. **열 편집**을 사용하여 변환할 열을 편집합니다. **규칙이 있는 **열을 선택하고 다음 목록을 복사하여 포함 열 이름 아래에 붙여넣습니다.  
...
1. 페이지 맨 위에서 **구성 및 제출**을 선택하여 **파이프라인 작업 설정** 대화 상자를 엽니다.

1. **기본** 페이지에서 **새로 만들기**를 선택하고 실험의 이름을 **mslearn-diabetes-training**으로 설정한 다음, **다음**을 선택합니다.
...
1. **검토 + 제출**을 선택하여 파이프라인 작업을 검토한 다음 **제출**을 선택하여 학습 파이프라인을 실행합니다.

1. 실행이 완료될 때까지 몇 분 정도 기다립니다. **자산**에서 **작업**을 선택하여 작업 상태를 검사할 수 있습니다. **mslearn-diabetes-training** 실험을 선택한 다음, **당뇨병 학습** 작업을 선택합니다.

## 변환된 데이터 보기

실행이 완료되면 이제 모델 학습을 위해 데이터 세트가 준비됩니다.

1. 캔버스의 **데이터 정규화** 모듈을 마우스 오른쪽 단추로 클릭하고(Mac에서는 Ctrl+클릭) **데이터 미리 보기**를 선택합니다. **변환된 데이터 집합**을 선택합니다.
...
1. **디자이너** 페이지로 돌아가서 **당뇨병 학습** 파이프라인을 선택합니다.
...
1. **자산 라이브러리**에서 **모델 학습** 모듈을 찾아 **데이터 분할** 모듈 아래의 캔버스에 배치합니다. 그런 다음 **데이터 분할** 모듈의 *Result dataset1*(왼쪽) 출력을 **모델 학습** 모듈의 *데이터 세트*(오른쪽)’ 입력에 연결합니다.
...
1. 자산에서 작업을 선택하여 **** 작업의 상태 확인합니다 **.** 실험 **mslearn-diabetes-training**을 선택한 다음, **당뇨병 학습** 파이프라인을 선택합니다.
...
1. 캔버스 위의 메뉴를 찾아 **유추 파이프라인 만들기**를 선택합니다. 메뉴에서 **유추 파이프라인 만들기**를 찾으려면 화면을 전체 화면으로 확장하고 화면의 오른쪽 위 모서리에 있는 세 개의 점 아이콘 **...** 을 클릭해야 할 수 있습니다.  
...
1. **데이터 세트에서 열 선택** 모듈을 편집합니다. *선택한 열*에서 **당뇨병** 환자를 제거합니다.
...
1. ****모델 체점** 모듈의 출력을 **Python 스크립트 실행**의 *Dataset1*(맨 왼쪽) 입력에 연결하고 **Python 스크립트 실행** 모듈의 결과 데이터 세트(왼쪽) 출력을 **웹 서비스 출력**에 연결합니다.
