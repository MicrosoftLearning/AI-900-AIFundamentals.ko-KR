...
1. [Azure Machine Learning 스튜디오](https://ml.azure.com?azure-portal=true) 화면 왼쪽 위에 있는 메뉴 아이콘을 선택하여 왼쪽 창을 확장합니다. **Designer** 페이지(작성 아래)**를** 보고 를 선택하여 **+** 새 파이프라인을 만듭니다.

1. 초안 이름(**Pipeline-Created-on-date****)을 **학습 펭귄 클러스터링**으로 변경합니다.
...
1. 왼쪽 메뉴에서 **디자이너**를 선택하여 파이프라인으로 돌아갑니다. **Designer** 페이지에서 **학습 펭귄 클러스터링** 파이프라인 초안을 선택합니다.
...
1. **데이터를** 선택하고, **펭귄 데이터 데이터** 세트를 검색하여 캔버스에 배치합니다.

1. 캔버스의 **penguin-data** 데이터 세트를 오른쪽 마우스 버튼으로 클릭(Mac에서는 Ctrl+클릭)하고 **데이터 미리 보기**를 클릭합니다.

1. 다양한 열의 분포를 히스토그램으로 볼 수 있음을 확인하여 *프로필* 탭을 선택합니다.
...
1. **저장**을 선택한 다음 데이터 **세트에서 열 선택** 메뉴를 닫고 디자이너 캔버스로 돌아갑니다.

1. **자산 라이브러리**에서 **누락된 데이터 정리** 모듈을 검색하여 데이터 **세트의 열 선택** 모듈 아래 캔버스에 배치하고 다음과 같이 연결합니다.

    ![데이터 세트의 열 선택 모듈을 누락된 데이터 정리 모듈의 연결하는 방법의 스크린샷.](media/create-clustering-model/clean-missing-data.png)

1. **누락된 데이터 정리** 모듈을 두 번 클릭하고 오른쪽의 설정 창에서 **열 편집**을 선택합니다. 그런 다음 **정리할 열** 창에서 **규칙 사용**을 선택하고 **모든 열**을 포함합니다.
...
1. **저장**을 선택한 다음 **데이터 정규화** 모듈 설정을 닫고 디자이너 캔버스로 돌아갑니다.

## 파이프라인 실행

데이터 변환을 적용하려면 파이프라인을 실험으로 실행해야 합니다.

1. 페이지 맨 위에서 **구성 & 제출** 을 선택하여 **파이프라인 작업 설정** 대화 상자를 엽니다.

1. **기본 페이지에서** **새로 만들기**를 선택하고 실험 이름을 **mslearn-penguin-training**으로 설정한 다음 **, 다음**을 선택합니다.

1. **입력 & 출력** 페이지에서 변경하지 않고 **다음**을 선택합니다.

1. **런타임 설정** 페이지에서 파이프라인을 실행할 기본 컴퓨팅이 없으면 오류가 나타납니다. **컴퓨팅 유형 선택** 드롭다운에서 *컴퓨팅 클러스터를* 선택하고 **Azure ML 컴퓨팅 클러스터 선택** 드롭다운에서 최근에 만든 컴퓨팅 클러스터를 선택합니다.

1. **다음**을 선택하여 파이프라인 작업을 검토한 다음 **제출**을 선택하여 학습 파이프라인을 실행합니다.

1. 실행이 끝날 때까지 기다립니다. 5분 이상 걸릴 수 있습니다. 자산**에서 작업을** 선택하여 작업의 상태 검사 수 **있습니다**. 여기에서 **펭귄 클러스터링 학습** 작업을 선택합니다.
...
1. **Transformed_dataset** 페이지를 닫고 파이프라인 실행으로 돌아갑니다.
...
1. **Designer** 페이지로 돌아가서 **펭귄 클러스터링 학습** 파이프라인 초안을 엽니다.
...
1. 학습 중인 모델은 특징을 사용하여 데이터를 클러스터로 그룹화하므로 클러스터링 알고리즘을 사용하여 모델을 학습해야 합니다. **자산 라이브러리**에서 **K-평균 클러스터링** 모듈을 검색하여 캔버스, **데이터 분할** 모듈의 왼쪽 및 **클러스터링 모델 학습** 모듈 위에 배치합니다. 그런 다음 출력을 **클러스터링 모델 학습** 모듈의 **학습되지 않은 모델**(왼쪽) 입력에 연결합니다.
...
1. **& 구성을** 선택하고 컴퓨팅 클러스터에서 **mslearn-penguin-training**이라는 기존 실험을 사용하여 파이프라인을 실행합니다.

1. 실행이 끝날 때까지 기다립니다. 5분 이상 걸릴 수 있습니다. 자산 **아래에서 작업을** 선택하여 작업의 상태 확인**합니다**. 여기에서 최신 **학습 펭귄 클러스터링** 작업을 선택합니다.

1. 실험 실행이 완료되면 **클러스터에 데이터 할당** 모듈을 마우스 오른쪽 단추로 클릭하고 **데이터 미리 보기를** 선택한 다음 **결과 데이터 세트를** 선택하여 결과를 확인합니다.
...
1. **Designer** 페이지에서 **학습 펭귄 클러스터링** 파이프라인 초안을 엽니다.
...
1. **& 제출 구성**을 선택하고 컴퓨팅 클러스터에서 **mslearn-penguin-training**이라는 기존 실험을 사용하여 파이프라인을 실행합니다.

1. 실험이 완료될 때까지 기다립니다. 상태 검사 **작업 페이지로** 이동하여 최신 **학습 펭귄 클러스터링** 작업을 선택합니다.

1. **모델 평가** 모듈을 마우스 오른쪽 단추로 클릭하고 **데이터 미리 보기를** 선택한 다음 **, 평가 결과를** 선택합니다. 각 행의 메트릭을 검토합니다.
    - **다른 중심까지의 평균 거리**
    - **클러스터 중심까지의 평균 거리**
    - **지점 수**
    - **클러스터 중심까지의 최대 거리**
...
1. 캔버스 위의 메뉴를 찾아 **유추 파이프라인 만들기를** 선택합니다. 메뉴에서 **유추 파이프라인 만들기**를 찾으려면 화면을 전체로 확장하고 화면의 오른쪽 위 모서리에 있는 **...** 아이콘을 클릭해야 할 수 있습니다.  

    ![유추 파이프라인 만들기의 위치 스크린샷](media/create-clustering-model/create-inference-pipeline.png)

1. **유추 파이프라인 만들기** 드롭다운 목록에서 **실시간 유추 파이프라인을** 선택합니다. 몇 초 후에 **Train Penguin Clustering-real time inference**라는 새 버전의 파이프라인이 열립니다.
...
1. 새 파이프라인의 이름을 **Predict Penguin Clusters**로 바꾸고 새 파이프라인을 검토합니다. 학습 파이프라인의 변환 및 클러스터링 모델이 이 파이프라인의 일부입니다. 학습된 모델은 새 데이터의 점수를 매기는 데 사용됩니다. 파이프라인에는 결과를 반환하는 웹 서비스 출력도 포함되어 있습니다.
    ...
    - **penguin-data 데이터** 세트를 **Species** 열을 포함하지 않는 **수동으로 데이터 입력** 구성 요소로 바꿉니다.
    - **데이터 세트 구성 요소에서 열 선택** 구성 요소를 제거합니다. 이제 중복됩니다.
    - **웹 서비스 입력**을 연결하고 데이터 수동 구성 요소(클러스터될 데이터의 입력을 나타낸)를 첫 번째 **변환 적용** 구성 요소에 **연결**합니다.

    파이프라인을 수정할 때 아래의 단계를 따르고 위 이미지와 정보를 참조합니다.

1. 파이프라인에는 사용자 지정 데이터 세트에서 만든 모델에 대한 **웹 서비스 입력** 구성 요소가 자동으로 포함되지 않습니다. 자산 라이브러리에서 **웹 서비스 입력** 구성 요소를 검색하여 파이프라인의 맨 위에 배치합니다.  **웹 서비스 입력** 구성 요소의 출력을 캔버스에 이미 있는 첫 번째 **변환 적용** 구성 요소의 *데이터 세트*(오른쪽) 입력에 연결합니다.  
...
1. 수동으로 **데이터 입력** 모듈의 출력을 첫 번째 **변환 적용** 모듈의 *데이터 세트*(오른쪽) 입력에 연결합니다.
...
1. **작업**으로 이동하여 **mslearn-penguin-inference** 실험 이름을 사용하여 최신 **펭귄 클러스터 예측** 작업을 선택합니다.

1. 파이프라인이 완료되면 **클러스터에 데이터 할당** 모듈을 마우스 오른쪽 단추로 클릭하고 **데이터 미리 보기를** 선택하고 **결과 데이터 세트를** 선택하여 입력 데이터의 세 가지 펭귄 관찰에 대해 예측된 클러스터 할당 및 메트릭을 확인합니다.
...
1. **펭귄 클러스터** 예측 유추 실행 페이지에서 위쪽 메뉴 모음에서 **배포**를 선택합니다.
