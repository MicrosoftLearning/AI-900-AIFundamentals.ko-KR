...
# Azure Machine Learning 디자이너를 사용하여 클러스터링 살펴보기
...
1. [Azure Machine Learning 스튜디오](https://ml.azure.com?azure-portal=true) 왼쪽 위에서 &#8801;** 아이콘(세 줄의 스택처럼 보이는 메뉴 아이콘)을 선택하여 **인터페이스의 다양한 페이지를 봅니다(화면 크기를 최대화해야 할 수 있음). 왼쪽 창에서 해당 페이지를 사용하여 작업 영역에서 리소스를 관리할 수 있습니다. **컴퓨팅** 페이지(**관리** 아래)를 선택합니다.
...
## 파이프라인 만들기 및 데이터 세트 추가

Azure Machine Learning 디자이너를 시작하려면 먼저 파이프라인을 만들어야 합니다.

1. [Azure Machine Learning 스튜디오](https://ml.azure.com?azure-portal=true) 화면 왼쪽 위에 있는 메뉴 아이콘을 선택하여 왼쪽 창을 확장합니다. 디자이너 페이지(작성 중 **)를** 보고 새 파이프라인을 만들도록 선택합니다**+**.** ** 

1. 초안 이름(**Pipeline-Created-on-date****)을 학습 펭귄 클러스터링으로 **변경합니다**.

    Azure Machine Learning에서 모델 학습 및 기타 작업의 데이터는 주로 데이터 세트라는 개체에 캡슐화됩니다. 이 모듈에서는 세 가지 종의 펭귄에 대한 관찰을 포함하는 데이터 세트를 사용합니다.

1. **데이터** 페이지(**자산** 아래)를 봅니다. 데이터 페이지에는 Azure ML에서 사용할 특정 데이터 파일 또는 테이블이 있습니다.
...
1. 왼쪽 메뉴에서 **디자이너**를 선택하여 파이프라인으로 돌아갑니다. **디자이너** 페이지에서 학습 펭귄 클러스터링 파이프라인 초안을** 선택합니다**.
...
1. 데이터를** 선택하고**, 펭귄 데이터 세트를** 검색하여 캔버스에 배치**합니다.

1. 캔버스의 **penguin-data** 데이터 세트를 오른쪽 마우스 버튼으로 클릭(Mac에서는 Ctrl+클릭)하고 **데이터 미리 보기**를 클릭합니다.

1. 프로필* 탭을 *선택하여 다양한 열의 분포를 히스토그램으로 볼 수 있음을 알 수 있습니다.
...
1. 파이프라인 캔버스에서 **데이터 세트를 볼 수 있도록 DataOutput** 페이지를 닫습니다.

## 변환 적용

1. 왼쪽의 **자산 라이브러리** 창에서 구성 요소 **(데이터 변환 및 모델 학습에 사용할 수 있는 다양한 모듈 포함)를 선택합니다**. 검색 창을 사용해 모듈을 빠르게 찾을 수도 있습니다.
...
1. **데이터 세트에서 열 선택** 모듈을 더블클릭하고 오른쪽에 있는 설정 창에서 **열 편집**을 선택합니다. 그런 다음 **열 선택** 창에서 **이름 기준**을 선택하고 **+** 링크를 사용하여 **CulmenLength**, **CulmenDepth**, **FlipperLength** 및 **BodyMass**를 선택합니다.

    ![열 이름 CulmenLength, CulmenDepth, FlipperLength 및 BodyMass를 포함하는 방법의 스크린샷.](media/create-clustering-model/select-columns.png)

1. 저장을 선택한 **다음 데이터 세트**의 **열 선택 메뉴를 닫아 디자이너 캔버스로** 돌아갑니다.

1. **자산 라이브러리**에서 누락된 데이터** 정리 모듈을 **검색하여 데이터 세트** 모듈의 **Select 열 아래에 있는 캔버스에 놓고 다음과 같이 연결합니다.

    ![데이터 세트의 열 선택 모듈을 누락된 데이터 정리 모듈의 연결하는 방법의 스크린샷.](media/create-clustering-model/clean-missing-data.png)

1. 누락된 데이터 정리 모듈을 **두 번 클릭하고 오른쪽의 설정 창에서 열** 편집을 선택합니다**.** 그런 다음 **정리할 열** 창에서 **규칙 사용**을 선택하고 **모든 열**을 포함합니다.

    ![규칙 포함 옵션을 사용하여 모든 열을 선택하는 방법의 스크린샷.](media/create-clustering-model/normalize-columns.png)

1. 저장**을 선택한 **다음, 설정 창에서 다음 구성 설정을 지정합니다.
    - **최소 누락 값 비율**: 0.0
    - **최대 누락 값 비율**: 1.0
    - **정리 모드**: 전체 행 제거
...
1. 저장을 선택한 **다음 데이터** 정규화 모듈 설정을 닫**아 디자이너 캔버스로** 돌아갑니다.
...
1. 기본 페이지에서 새로** 만들기를 선택하고 **실험의 이름을 mslearn-penguin-training**으로 **설정한 다음, 다음**을 선택합니다**.** ** 

1. **입력 및 출력** 페이지에서 변경하지 않고 다음**을 선택합니다**.

1. **런타임 설정** 페이지에서 파이프라인을 실행할 기본 컴퓨팅이 없으면 오류가 표시됩니다. **컴퓨팅 유형** 선택 드롭다운에서 컴퓨팅 클러스터*를 선택하고 *Azure ML 컴퓨팅 클러스터** 선택 드롭다운에서 **최근에 만든 컴퓨팅 클러스터를 선택합니다.

1. 다음을 선택하여 **파이프라인 작업을 검토한 다음 제출**을 선택하여 **학습 파이프라인을 실행**합니다.

1. 실행이 끝날 때까지 기다립니다. 5분 이상 걸릴 수 있습니다. 자산에서 작업을 선택하여 **** 작업의 상태 검사 수 있습니다 **.** 여기에서 펭귄 클러스터링** 학습 작업을 선택합니다**.


## 변환된 데이터 보기

1. 실행이 완료되면 모듈은 다음과 같이 표시됩니다.

    ![각 모듈의 왼쪽에 녹색 막대가 있는 완료된 상태의 모듈 스크린샷.](media/create-clustering-model/normalize-complete.png)

1. 데이터** 정규화 모듈을 **마우스 오른쪽 단추로 클릭하고 데이터** 미리 보기를 선택한 **다음 변환된 데이터 세트를** 선택하여 **결과를 봅니다.

1. 데이터를 보면 **Species** 열이 제거되어 누락된 값이 없고 4가지 모든 특징에 대한 값이 일반적인 규모로 정규화되었음을 알 수 있습니다.

1. 파이프라인 실행으로 **돌아가려면 Transformed_dataset** 페이지를 닫습니다.
...
1. 디자이너** 페이지로 돌아가**서 학습 펭귄 클러스터링 파이프라인 초안을** 엽니다**.
...
1. **자산 라이브러리**에서 **클러스터링 모델 학습** 모듈을 찾아 **데이터 분할** 모듈 아래의 캔버스에 배치합니다. 그런 다음 분할 데이터** 모듈의 **결과 데이터 세트1 *(왼쪽) 출력을 *클러스터링 모델** 학습 모듈의 데이터 세트*(오른쪽) 입력에 **연결*합니다.
...
1. 학습 중인 모델은 특징을 사용하여 데이터를 클러스터로 그룹화하므로 클러스터링 알고리즘을 사용하여 모델을 학습해야 합니다. **자산 라이브러리**에서 K-평균 클러스터링 모듈을** 검색하여 캔버스, 분할 데이터** 모듈 왼쪽 **및 클러스터링 모델** 학습 모듈 위에 **배치**합니다. 그런 다음 출력을 **클러스터링 모델 학습** 모듈의 **학습되지 않은 모델**(왼쪽) 입력에 연결합니다.
...
1. K-평균 알고리즘은 항목을 지정한 클러스터의 수, 즉 ***K*** 값으로 그룹화합니다. **K-평균 클러스터링** 모듈을 선택해 오른쪽 창에서 **중심 수** 매개 변수를 **3**으로 설정합니다.
    ...
    > 5. c 단계를 반복합니다. 및 d. 클러스터 할당이 안정화되거나 지정된 반복 횟수가 완료될 때까지
...
1. 구성 및 제출을 선택하고 **컴퓨팅 클러스터에서 mslearn-penguin-training**이라는 **기존 실험을 사용하여 파이프라인을 실행**합니다.

1. 실행이 끝날 때까지 기다립니다. 5분 이상 걸릴 수 있습니다. 자산에서 작업을 선택하여 **** 작업의 상태 확인합니다 **.** 여기에서 최신 **학습 펭귄 클러스터링** 작업을 선택합니다.

1. 실험 실행이 완료되면 클러스터에 **데이터 할당 모듈을 마우스 오른쪽 단추로 클릭하고 데이터 미리 보기를 선택한 **다음 결과 데이터** 세트를** 선택하여 **결과를 확인**합니다.
...
1. Results_dataset** 시각화를 **닫고 파이프라인 실행으로 돌아갑니다.
...
1. **디자이너** 페이지에서 펭귄 클러스터링 학습 파이프라인 초안을** 엽니다**.
...
1. 실험이 완료될 때까지 기다립니다. 상태 검사 작업 페이지로 **** 이동하여 최신 **학습 펭귄 클러스터링** 작업을 선택합니다.

1. 모델 평가 모듈을 **마우스 오른쪽 단추로 클릭하고 미리 보기 데이터를** 선택한 **다음 평가 결과를** 선택합니다**.** 각 행의 메트릭을 검토합니다.
    - **다른 중심까지의 평균 거리**
    - **클러스터 중심까지의 평균 거리**
    - **지점 수**
    - **클러스터 중심까지의 최대 거리**

1. Evaluation_results** 탭을 **닫습니다.
...
1. 캔버스 위의 메뉴를 찾아 유추 파이프라인** 만들기를 선택합니다**. 메뉴에서 유추 파이프라인** 만들기를 찾으**려면 화면을 전체 화면으로 확장하고 화면의 오른쪽 위 모서리에 있는 ...** 아이콘을 클릭**해야 할 수 있습니다.  

    ![유추 파이프라인 만들기의 위치 스크린샷](media/create-clustering-model/create-inference-pipeline.png)

1. **유추 파이프라인** 만들기 드롭다운 목록에서 실시간 유추 파이프라인**을 선택합니다**. 몇 초 후에 **Train Penguin Clustering-real time inference**라는 새 버전의 파이프라인이 열립니다.

1. 새 파이프라인의 이름을 **Predict Penguin Clusters**로 바꾸고 새 파이프라인을 검토합니다. 학습 파이프라인의 변환 및 클러스터링 모델이 이 파이프라인의 일부입니다. 학습된 모델은 새 데이터의 점수를 매기는 데 사용됩니다. 파이프라인에는 결과를 반환하는 웹 서비스 출력도 포함되어 있습니다.
    ...
    - **펭귄 데이터 데이터** 세트를 **Species** 열을 포함하지 않는 Enter Data Manually** 구성 요소로 **바꿉니다.
    - **데이터 세트** 구성 요소에서 열 선택(이제 중복)을 제거합니다.
    - ** 커넥트첫 번째 **변환** 적용 구성 요소에 대한 웹 서비스 입력** 및 **데이터 수동** 구성 요소(클러스터형 데이터에 대한 입력을 나타낸)를 입력합니다.

    파이프라인을 수정할 때 아래의 단계를 따르고 위 이미지와 정보를 참조합니다.

1. 파이프라인에는 사용자 지정 데이터 세트에서 만든 모델에 대한 **웹 서비스 입력** 구성 요소가 자동으로 포함되지 않습니다. 자산 라이브러리에서 **웹 서비스 입력** 구성 요소를 검색하여 파이프라인의 맨 위에 배치합니다.  의 출력 ** 을 커넥트캔버스에 이미 있는 *첫 번째 **변환** 적용 구성 요소의 데이터 세트*(오른쪽) 입력에 대한 웹 서비스 입력** 구성 요소입니다.  

1. 추론 파이프라인은 새 데이터가 원래 학습 데이터의 스키마와 일치하는 것으로 가정하므로 학습 파이프라인의 **penguin-data** 데이터 세트가 포함됩니다. 하지만 이 입력 데이터에는 모델에서 사용하지 않는 펭귄 종에 대한 열이 포함되어 있습니다. **penguin-data** 데이터 세트와 **데이터 세트에서 열 선택** 모듈을 모두 삭제하고 **자산 라이브러리**에 있는 **수동으로 데이터 입력** 모듈로 대체합니다.

1. 그런 다음 **수동으로 데이터 입력** 모듈의 설정을 수정하여 다음 CSV 입력을 사용합니다. 여기에는 3개의 새로운 펭귄 관찰에 대한 특징 값이 포함되어 있습니다(헤더 포함).
...
1. 의 출력 ** 을 커넥트첫 번째 **변환** 적용 모듈의 *데이터 세트*(오른쪽) 입력에 Data Manually** 모듈을 입력합니다.
...
1. 작업**으로 **이동하여 mslearn-penguin-inference** 실험 이름을 사용하여 **최신 **Predict Penguin 클러스터** 작업을 선택합니다.

1. 파이프라인이 완료되면 클러스터**에 데이터 할당 모듈을 마우스 오른쪽 단추로 클릭하고**, 데이터** 미리 보기를 선택하고**, 결과 데이터 세트를** 선택하여 **입력 데이터의 3가지 펭귄 관찰에 대한 예측 클러스터 할당 및 메트릭을 확인합니다.
...
1. 배포 상태 보려면 화면 왼쪽 위에 있는 메뉴 아이콘을 선택하여 왼쪽 창을 확장합니다. **엔드포인트** 페이지(**자산** 아래)를 보고 **predict-penguin-clusters**를 선택합니다. 배포가 완료되면 **배포 상태**가 **정상**으로 변경됩니다.
