...
이 연습에서는 Azure Machine Learning의 자동화된 기계 학습 기능을 사용하여 기계 학습 모델을 학습시키고 평가합니다. 그런 다음, 학습된 모델을 배포하고 테스트합니다.
...
1. Azure Machine Learning 스튜디오에서 새로 만든 작업 영역이 표시됩니다. 그렇지 않은 경우 왼쪽 메뉴에서 **모든 작업 영역을** 선택하고, Hen은 방금 만든 작업 영역을 선택합니다.

## 미리 보기 기능 사용

Azure Machine Learning의 일부 기능은 미리 보기 상태이며 작업 영역에서 명시적으로 사용하도록 설정해야 합니다.

1. Azure Machine Learning Studio에서 **미리 보기 기능 관리를** 클릭합니다(스피커 아이콘 - &#128363;).
...
자동화된 기계 학습을 사용하면 여러 알고리즘 및 매개 변수를 시도하여 여러 모델을 학습시키고 데이터에 가장 적합한 모델을 식별할 수 있습니다. 이 연습에서는 기록 자전거 대여 세부 정보의 데이터 세트를 사용하여 계절 및 기상 기능에 따라 지정된 날에 예상되는 자전거 대여 수를 예측하는 모델을 학습시킵니다.
   ...
   **작업 유형 & 데이터**:
    ...
    **작업 설정**:
    ...
    - **제한**: *이 섹션 확장*
        ...
        - **최대 동시 평가판**: 3
        - **최대 노드**: 3
        - **메트릭 점수 임계값**: 0.85(*모델이 정규화된 루트 평균 제곱 오차 메트릭 점수 0.085 이하를 달성하면 작업이 종료됩니다.*)
        - **시간 제한**: 15
        - **반복 시간 제한**: 5
        - **조기 종료 사용**: *선택됨*
    - **유효성 검사 및 테스트**:
...
1. **메트릭** 탭을 선택하고 **잔차** 및 **predicted_true** 차트를 아직 선택하지 않은 경우 선택합니다. 
    ...
    모델의 성능을 보여 주는 차트를 검토합니다. **잔차** 차트는 *잔차*(예측 값과 실제 값의 차이)를 히스토그램으로 표시합니다. **predicted_true** 차트는 예측된 값을 실제 값과 비교합니다. 

## 모델 배포 및 테스트

1. 자동화된 기계 학습 작업에서 학습된 최상의 모델을 위한 **모델** 탭에서 **배포** 를 선택하고 **웹 서비스** 옵션을 사용하여 다음 설정을 사용하여 모델을 배포합니다.
    - **이름**: predict-rentals
    - **설명**: 자전거 대여 예측
...
1. **엔드포인트를 테스트할 입력 데이터** 창에서 템플릿 JSON을 다음 입력 데이터로 바꿉다.
...
1. 다음과 같이 입력 기능에 따라 예측된 대여 수를 포함하는 테스트 결과를 검토합니다.
