...
1. 초안 이름(**Pipeline-Created-on-date****)을 **Auto Price Training**으로 변경합니다.
...
1. 실행이 완료될 때까지 몇 분 정도 기다립니다. 자산**에서 작업을** 선택하여 작업의 상태 검사 수 **있습니다**. 여기에서 **자동 가격 학습** 작업을 선택합니다.
...
1. 실험 실행이 완료되면 **모델 평가** 모듈을 마우스 오른쪽 단추로 클릭하고 **데이터 미리 보기** , **평가 결과를** 차례로 선택합니다.
...
1. 유추 파이프라인은 새 데이터가 원래 학습 데이터의 스키마와 일치하는 것으로 가정하므로 학습 파이프라인의 **자동차 가격 데이터(원시)** 데이터 세트가 포함됩니다. 하지만 해당 입력 데이터에는 모델이 예측하는 **가격** 레이블이 포함되어 있으므로, 아직 가격 예측이 이루어지지 않은 새 자동차 데이터에 이를 포함시키는 것은 비논리적입니다. 이 모듈을 삭제하고 데이터 **입력 및 출력** 섹션에서 **수동으로 데이터 입력** 모듈로 바꿉 있습니다.
1. **수동으로 데이터 입력** 모듈을 편집하고 다음 CSV 데이터를 입력합니다. 여기에는 세 대의 자동차에 대한 레이블이 없는 기능 값이 포함됩니다(전체 텍스트 블록 복사 및 붙여넣기).
...
1. **모델 점수 매기**기 모듈의 출력을 **Python 스크립트 실행**의 **Dataset1**(가장 왼쪽) 입력에 연결합니다.
...
1. **작업** 페이지로 돌아가서 최신 **자동 가격 학습** 작업 실행(*mslearn-auto-inference 실험*과 관련된 작업 실행)을 선택합니다.
...
1. **predict-auto-price** 엔드포인트가 열리면 **테스트** 탭을 선택합니다. 이 탭은 새 데이터로 모델을 테스트하는 데 사용합니다. **엔드포인트를 테스트하기 위해 입력 데이터**에서 현재 데이터를 삭제합니다. 아래 데이터를 복사하여 데이터 섹션에 붙여넣습니다.  
