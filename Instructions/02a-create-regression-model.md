...
1. 초안 이름(**Pipeline-Created-on-date****)을 자동 가격 교육**으로 **변경합니다.
...
1. 구성 요소를** 선택합니다**. **자동차 가격 데이터(원시)** 데이터 세트를 찾아 캔버스에 배치합니다.

1. 캔버스에서 자동차 가격 데이터(원시) **데이터 세트를 마우스 오른쪽 단추로 클릭하고(Mac에서 Ctrl+클릭)** 데이터** 미리 보기를 선택합니다**.
...
1. 다음과 같이 캔버스에서 **데이터 세트를 볼 수 있도록 DataOutput** 창을 닫습니다.
...
1. 저장**을 선택하고 **소유 창을 닫습니다.

이 연습의 나머지 단계에서는 다음과 같은 파이프라인을 만드는 단계를 진행합니다.
...
1. 누락된 데이터** 정리 모듈을 **두 번 클릭하고 오른쪽 창에서 열** 편집을 선택합니다**. **정리할 열** 창에서 **규칙 사용**을 선택하고, **포함** 목록에서 **열 이름**을 선택한 다음, 열 이름 상자에 다음과 같이 **bore**, **stroke**, **horsepower**를 입력합니다.
...
1. **기본 페이지에서 새로** 만들기를** 선택하고 **실험**의 이름을 mslearn-auto-training**으로 설정한 다음, 다음**을 선택합니다**.
...
1. 실행이 완료될 때까지 몇 분 정도 기다립니다. 자산에서 작업을 선택하여 **** 작업의 상태 검사 수 있습니다 **.** 여기에서 자동 가격 학습** 작업을 선택합니다**. 여기에서 작업이 완료된 시기를 확인할 수 있습니다. 작업이 완료되면 이제 모델 학습을 위해 데이터 세트가 준비됩니다.

1. 왼쪽 메뉴로 이동합니다. 작성에서 **디자이너**를** 선택합니다**. 그런 다음, 파이프라인 목록에서 **자동 가격 학습* 파이프라인을** 선택합니다*.

## 학습 파이프라인 만들기

데이터 변환을 사용하여 데이터를 준비한 후에는 이를 사용하여 기계 학습 모델을 학습할 수 있습니다. 다음 단계를 수행하여 **자동차 가격 학습** 파이프라인을 확장합니다.

1. 왼쪽 메뉴에 디자이너**가 **선택되어 있고 자동 가격 학습** 파이프라인으로 돌아**갔는지 확인합니다.
...
1. 구성 및 제출**을 선택하고 **mslearn-auto-training**이라는 **기존 실험을 사용하여 파이프라인을 실행합니다.

1. 실험 실행을 완료하는 데는 5분 이상 걸립니다. 작업** 페이지로 돌아가**서 최신 **자동 가격 학습** 작업 실행을 선택합니다.

1. 실험 실행이 완료되면 모델** 점수 매기기 모듈을 마우스 오른쪽 단추로 클릭하고 **미리 보기 데이터를** 선택한 다음 **점수가 매기된 데이터 세트를** 선택하여 **결과를 봅니다.
...
회귀 모델을 평가하는 한 가지 방법은 예측 레이블을 학습 중에 보류된 유효성 검사 데이터 세트의 실제 레이블과 비교하는 것입니다. 또 다른 방법은 여러 모델의 성능을 비교하는 것입니다.
...
1. 실험 실행을 완료하는 데 몇 분 정도 걸립니다. 작업** 페이지로 돌아가**서 최신 **자동 가격 학습** 작업 실행을 선택합니다.
...
1. **유추 파이프라인** 만들기 드롭다운 목록에서 실시간 유추 파이프라인**을 선택합니다**. 몇 초 후에 **자동차 가격 학습-실시간 유추**라는 파이프라인의 새 버전이 열립니다.
...
1. 새 파이프라인의 이름을 **자동차 가격 예측**으로 바꾸고 새 파이프라인을 검토합니다. 여기에는 제출할 새 데이터에 대한 웹 서비스 입력과 결과를 반환하기 위한 웹 서비스 출력이 포함되어 있습니다. 일부 변환 및 학습 단계가 이 파이프라인의 일부입니다. 학습된 모델은 새 데이터의 점수를 매기는 데 사용됩니다.
    ...
    다음 단계에서 유추 파이프라인을 다음과 같이 변경합니다.

    ![변경 내용이 표시된 유추 파이프라인의 스크린샷](media/create-regression-model/inference-changes.png)

   다음 단계에서 파이프라인을 수정할 때 참조용 이미지를 사용합니다.

1. 유추 파이프라인은 새 데이터가 원래 학습 데이터의 스키마와 일치하는 것으로 가정하므로 학습 파이프라인의 **자동차 가격 데이터(원시)** 데이터 세트가 포함됩니다. 하지만 해당 입력 데이터에는 모델이 예측하는 **가격** 레이블이 포함되어 있으므로, 아직 가격 예측이 이루어지지 않은 새 자동차 데이터에 이를 포함시키는 것은 비논리적입니다. 이 모듈을 삭제하고 데이터 입력 및 출력** 섹션에서 수동으로** 데이터 입력 모듈**로 바꿉**다.
1. 데이터** 수동 입력 모듈을 **편집하고 세 대의 자동차에 대한 레이블이 없는 기능 값을 포함하는 다음 CSV 데이터를 입력합니다(전체 텍스트 블록 복사 및 붙여넣기).
...
1. **모델 채점** 모듈의 출력에는 모든 입력 특징 및 예측된 레이블도 포함됩니다. 예측만 포함하도록 출력을 수정하려면 다음을 수행합니다.
    ...
    ```
>**참고**: 복사 및 붙여넣기에서는 Python 스크립트에 공백을 도입할 수 있습니다. 가져오*거나 데프*하거나 **반환*하기 전에 *공백이 없다는 것을 두 번 검사. scored_results 및 scored_results.rename**()* 전에 *탭 들여쓰기 하나가 있는지 확인합니다.

1. 에서 출력 ** 을 커넥트모델** 모듈을 Python 스크립트** 실행의 **Dataset1**(가장 왼쪽) 입력에 **채점합니다.

1. ** 커넥트Python 스크립트** 실행 모듈의 **결과 데이터 세트**(왼쪽)를 웹 서비스 출력 모듈에 **출력**합니다.
...
1. 작업** 페이지로 돌아가**서 최신 **자동 가격 학습** 작업 실행(mslearn-auto-inference 실험*과 관련된 *작업)을 선택합니다.

1. 파이프라인이 완료되면 Python 스크립트** 실행 모듈을 마우스 오른쪽 단추로 **클릭합니다. **데이터 미리 보기**를 선택한 다음 **결과 데이터 세트**를 선택하여 입력 데이터의 자동차 3대에 대한 예상 가격을 확인합니다.

1. Result_Dataset** 탭을 **닫습니다.
...
1. **predict-auto-price** 엔드포인트가 열리면 **테스트** 탭을 선택합니다. 이 탭은 새 데이터로 모델을 테스트하는 데 사용합니다. 입력 데이터 아래 **의 현재 데이터를 삭제하여 엔드포인트를 테스트합니다**. 아래 데이터를 복사하여 데이터 섹션에 붙여넣습니다.  
