...
## Designer 파이프라인 만들기 및 데이터 세트 추가
...
1. 왼쪽의 **자산 라이브러리** 창에서 **구성 요소를** 선택합니다. 여기에는 데이터 변환 및 모델 학습에 사용할 수 있는 다양한 모듈이 포함되어 있습니다. 검색 창을 사용해 모듈을 빠르게 찾을 수도 있습니다.
...
1. **누락된 데이터 정리** 모듈을 두 번 클릭하고 오른쪽 창에서 **열 편집**을 선택합니다. **정리할 열** 창에서 **규칙 사용**을 선택하고, **포함** 목록에서 **열 이름**을 선택한 다음, 열 이름 상자에 다음과 같이 **bore**, **stroke**, **horsepower**를 입력합니다.
...
1. **기본 페이지에서** **새로 만들기**를 선택하고 실험 이름을 **mslearn-auto-training으로** 설정한 다음 **, 다음**을 선택합니다.
...
1. 실행이 완료될 때까지 몇 분 정도 기다립니다. 자산**에서 작업을** 선택하여 작업의 상태 검사 수 **있습니다**. 여기에서 **자동 가격 학습** 작업을 선택합니다. 여기에서 작업이 완료된 시점을 확인할 수 있습니다. 작업이 완료되면 이제 모델 학습을 위해 데이터 세트가 준비됩니다.

1. 왼쪽 메뉴로 이동합니다. **작성에서** **Designer** 선택합니다. 그런 다음, 파이프라인 목록에서 *Auto Price Training* **파이프라인을** 선택합니다.

## 학습 파이프라인 만들기

데이터 변환을 사용하여 데이터를 준비한 후에는 이를 사용하여 기계 학습 모델을 학습할 수 있습니다. 다음 단계를 수행하여 **자동차 가격 학습** 파이프라인을 확장합니다.

1. 왼쪽 메뉴가 **Designer** 선택되어 있고 **자동 가격 학습** 파이프라인으로 돌아갔는지 확인합니다.
...
1. **& 구성을** 선택하고 **mslearn-auto-training**이라는 기존 실험을 사용하여 파이프라인을 실행합니다.

1. 실험 실행을 완료하는 데는 5분 이상 걸립니다. **작업** 페이지로 돌아가서 최신 **자동 가격 학습** 작업 실행을 선택합니다.

1. 실험 실행이 완료되면 **모델 점수 매기** 기 모듈을 마우스 오른쪽 단추로 클릭하고 **데이터 미리 보기를** 선택한 다음 **점수가 매기된 데이터 세트를** 선택하여 결과를 봅니다.
...
1. 실험 실행을 완료하는 데 몇 분 정도 걸립니다. **작업** 페이지로 돌아가서 최신 **자동 가격 학습** 작업 실행을 선택합니다.
...
1. **유추 파이프라인 만들기** 드롭다운 목록에서 **실시간 유추 파이프라인을** 선택합니다. 몇 초 후에 **자동차 가격 학습-실시간 유추**라는 파이프라인의 새 버전이 열립니다.
...
1. 유추 파이프라인은 새 데이터가 원래 학습 데이터의 스키마와 일치하는 것으로 가정하므로 학습 파이프라인의 **자동차 가격 데이터(원시)** 데이터 세트가 포함됩니다. 하지만 해당 입력 데이터에는 모델이 예측하는 **가격** 레이블이 포함되어 있으므로, 아직 가격 예측이 이루어지지 않은 새 자동차 데이터에 이를 포함시키는 것은 비논리적입니다. 이 모듈을 삭제하고 데이터 **입력 및 출력** 섹션에서 **수동으로 데이터 입력** 모듈로 바꿉다.
1. **수동으로 데이터 입력** 모듈을 편집하고 세 대의 자동차에 대한 레이블이 없는 기능 값을 포함하는 다음 CSV 데이터를 입력합니다(전체 텍스트 블록을 복사하여 붙여넣기).
...
1. **모델 채점** 모듈의 출력에는 모든 입력 특징 및 예측된 레이블도 포함됩니다. 예측만 포함하도록 출력을 수정하려면 다음을 수행합니다.
    ...
    ```
>**참고**: 복사 및 붙여넣기를 사용하면 Python 스크립트에 공백이 포함될 수 있습니다. *가져오기* 또는 *def* 또는 *return* 전에 공백이 없다는 것을 이중 검사. scored_results 및 *scored_results.rename()* 앞에 하나의 탭 ** 들여쓰기 가 있는지 확인합니다.

1. **모델 점수 매기**기 모듈의 출력을 **Python 스크립트 실행**의 **Dataset1**(왼쪽) 입력에 연결합니다.

1. **Python 스크립트 실행** 모듈의 **결과 데이터 세트**(왼쪽) 출력을 **웹 서비스 출력** 모듈에 연결합니다.
...
1. **작업** 페이지로 돌아가서 최신 **자동 가격 학습** 작업 실행(*mslearn-auto-inference 실험*과 관련된 작업)을 선택합니다.
...
1. **predict-auto-price** 엔드포인트가 열리면 **테스트** 탭을 선택합니다. 이 탭은 새 데이터로 모델을 테스트하는 데 사용합니다. 입력 데이터에서 현재 데이터를 삭제 **하여 엔드포인트를 테스트합니다**. 아래 데이터를 복사하여 데이터 섹션에 붙여넣습니다.  
